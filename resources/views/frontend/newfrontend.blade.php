    ...
    <script>
        ...
        function getPotensi (result) {
            ...
            url: geoJsonUrl,
                ...
                onEachFeature: function (feature, layer) {
                    // console.log(feature.geometry.coordinates.getBounds());
                    // var polygon = turf.random('polygon').features[0];
                    // let result = turf.pointsWithinPolygon(turf.points([points.coordinates]), turf.polygon(features.geometry.coordinates));
                    result.forEach(res => {
                            ...
                            if (res.kecamatan == feature.properties.kecamatan) {
                                layer.bindPopup( `<p style="font-weight: bold;"> `+res.kecamatan +`</p>`+`<p> Potensi `+ res.potensi +`</p><p> Bulan: `+ res.triwulan +`</p>`+`<p> Jumlah Kasus DBD: `+ res.kasus_dbd +`</p>`).addTo(potensi);
                                layer.setStyle(getColorPotensi(res.potensi));
                                var bounds = layer.getBounds();
                                for (let i = 0; i < res.jumlahRumahPositif; i++) {
                                    // get max and min all layer
                                    var southWest = bounds.getSouthWest();
                                    var northEast = bounds.getNorthEast();
                                    var lngSpan = northEast.lng - southWest.lng;
                                    var latSpan = northEast.lat - southWest.lat;
                                    // var point = [southWest.lat + latSpan * Math.random(),southWest.lng + lngSpan * Math.random()];
                                    // var point = [southWest, northEast];
